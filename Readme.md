...
- Evidence collection is read-only. Live Emby writes occur only after an operator opens a case, reviews its complete final identity/media projection, resolves any genuine correction questions, and presses the case-specific **Apply** button.
...
The full-screen evidence dialog is the searchable, sortable, filterable case browser. Its **Open case** Boolean command opens a full-screen master/detail projection comparing current and resulting Emby people and provider IDs; collapsed child rows show every relevant media credit as keep, move, receive or correction-required. Identity and media correction Boolean commands open contextual choice dialogs that save durable `provider_correction` rows and recalculate the same case. Apply is a normal dialog `ButtonItem`, not a built-in OK handle.
...
Schema migrations are offline operations. Stop Emby and back up `entity-resolution.db` before applying every migration after the database's current `schema_info.version`, in numeric order. A schema-9 workspace requires `010_case_credit_attribution.sql`; an older workspace must apply the earlier numbered files first. Use SQLite's fail-fast mode, for example `sqlite3.exe -bail entity-resolution.db ".read C:/path/to/PersonCleaner/migrations/010_case_credit_attribution.sql"`. The plugin validates `schema_info` before touching the existing structure and refuses to start resolution against an older or incomplete schema. Run the evidence task once after migration so provider-owner case rows and outcome plans are populated.
