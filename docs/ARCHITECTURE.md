...
The **Open case** checkbox opens the holistic master/detail review dialog using the existing dialog replacement/parent-return lifecycle. Identity parents compare current and resulting Emby/provider IDs. Media rows show the native TMDB owner, native TVDB owner and whether those assertions agree on the final identity and role; keep/move/receive is a separate Emby action. There is no separate save-plan lifecycle. As soon as the edited layout is structurally valid, the top-level **Apply** `ButtonItem` is available, including when Emby already matches the confirmed result. Apply validates the original evidence hash and live Emby snapshot, performs only the listed ID/credit operations, and atomically records the apply receipt plus the smallest set of contextual correction choices needed to reproduce the confirmed result on future runs. It does not serialize the complete person layout as identity-relation, local-binding or local-credit rules. A zero-mutation Apply is still a real confirmation and receipt. Interactive apply does not rebuild the whole provider graph; the next evidence task applies the new rules and refreshes related cases.
