...
namespace PersonCleaner.Tvdb
{
    ...
    internal sealed class TvdbIdentityResolver
    {
        ...
        public TvdbIdentityResolver(TvdbApiClient api, ILibraryManager library) { this.api = api; this.library = library; }
        ...
        private async Task<ResolutionResult> ResolveName(BaseItem item, string type, CancellationToken ct)
        {
            ...
            foreach (var candidate in all.Take(10))
            {
                ...
                if (localFilmography != null && localFilmography.Count > 0 && int.TryParse(candidate.tvdb_id, out var personTvdbId))
                {
                    ...
                    var overlap = localFilmography.Count(x => remoteFilmography.Contains(x));
                    var overlapIds = localFilmography.Where(x => remoteFilmography.Contains(x)).OrderBy(x => x).ToArray();
                    if (overlap >= 3) score += 0.35;
                    else if (overlap == 2) score += 0.30;
                    else if (overlap == 1) score += 0.20;
                    structural += "; local_known_productions=" + localFilmography.Count + "; local_production_ids=[" + string.Join(",", localFilmography.OrderBy(x => x)) + "]" +
                                  "; tvdb_productions=" + remoteFilmography.Count + "; filmography_overlap=" + overlap + "; overlap_ids=[" + string.Join(",", overlapIds) + "]";
                    candidateEvidence.Add(new ResolutionCandidate { TvdbId = candidate.tvdb_id, EntityType = type, Name = candidate.name, Score = score, ExternalIds = candidatePerson.remoteIds ?? new List<RemoteIdData>(), FilmographyIds = remoteFilmography.OrderBy(x => x).ToList(), Evidence = structural.TrimStart(';', ' ') });
                }
                else candidateEvidence.Add(new ResolutionCandidate { TvdbId = candidate.tvdb_id, EntityType = type, Name = candidate.name, Score = score, ExternalIds = candidate.remote_ids ?? new List<RemoteIdData>(), Evidence = structural.TrimStart(';', ' ') });
        ...
        private HashSet<string> GetLocalPersonProductionIds(Person person, CancellationToken ct)
        {
            ...
            foreach (var media in items)
            {
                ...
                var tvdb = media.GetProviderId(MetadataProviders.Tvdb);
